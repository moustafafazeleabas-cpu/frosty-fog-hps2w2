...
const lancerImpression = (type, data, params) => {
  ...
  if (!win) { alert("⚠️ Navigateur bloqué. Autorisez les Pop-ups."); return; }
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row font-sans text-gray-800">
      ...
      <nav className={`fixed inset-y-0 left-0 z-[70] w-72 bg-[#800020] text-white p-6 shadow-2xl flex flex-col transition-transform transform ${menuOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0`}>
        <div className="flex flex-col h-full">
          ...
          <div className="flex flex-col gap-1 flex-1 overflow-y-auto custom-scrollbar">
            <NavBtn active={view==='messagerie'} onClick={()=>changeView('messagerie')}>✉️ Messagerie {msgNonLus > 0 && <span className="ml-2 bg-red-500 px-2 rounded-full">{msgNonLus}</span>}</NavBtn>
      ...
      <main className="flex-1 p-4 md:p-8 overflow-y-auto relative">
        ...
        {(view==='caisse' || view==='facture_a4' || view==='devis' || view==='admin_credit') && <ModuleVente mode={view} params={parametres} categoriesDb={categoriesDb} />}
...
const ModuleVente = ({ mode, params, categoriesDb }) => {
  ...
  const updateRemise = (id, val) => {
    setPanier(panier.map(i => i.id === id ? {...i, remise_montant: safeNum(val)} : i));
  };

  const valider = async () => {
    if (panier.length === 0 || !selClient) return alert("Vérifiez panier et client");
    ...
    const typeToDb = mode === 'devis' ? 'devis' : 'historique_ventes';
    const finalType = mode === 'caisse' ? 'CAISSE' : mode.toUpperCase();
    const numero = (mode === 'devis' ? 'DV' : 'FA') + Date.now().toString().slice(-6);

    if (mode === 'devis') {
        await supabase.from('devis').insert([{ numero_devis: numero, client_nom: selClient, articles_liste: panier.map(i=>`${i.qte}x ${i.nom}`).join(', '), montant_total: totalNet, details_json: details }]);
    } else {
        await supabase.from('historique_ventes').insert([{ numero_facture: numero, type_vente: finalType, client_nom: selClient, montant_total: totalNet, articles_liste: panier.map(i=>`${i.qte}x ${i.nom}`).join(', '), methode_paiement: methode, details_json: details }]);
        if (mode === 'admin_credit') {
            await supabase.from('credits').insert([{ numero_facture: numero, nom_client: selClient, montant_du: totalNet, details_articles: panier.map(i=>`${i.qte}x ${i.nom}`).join(', '), statut: 'non_paye', details_json: details }]);
        }
    }

    lancerImpression(mode, { numero, panier, totalNet, fraisLivraison: isLiv ? fraisLiv : 0, client_nom: selClient, date: new Date(), printSize: mode==='caisse'?'58mm':'A4' }, params);
    setPanier([]); load(); alert("Transaction Terminée !");
  };

  return (
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 h-full">
      <div className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-gray-100 flex flex-col h-[75vh]">
      ...
      <div className="bg-[#800020] text-white p-6 rounded-[3rem] shadow-2xl flex flex-col h-[75vh] relative overflow-hidden">
        <div className="mb-4">
          <h3 className="font-black uppercase tracking-tighter mb-4 border-b border-white/20 pb-2">Opération : {mode.replace('_',' ')}</h3>
          {mode === 'caisse' ? (
             <div className="w-full p-3 bg-white/10 rounded-xl font-black text-center text-xs border border-white/20">VENTE À CONSOMMATEUR</div>
          ) : (
        ...
        <div className="flex-1 overflow-y-auto space-y-2 custom-scrollbar pr-2">
          ...
          {panier.map(item => (
            <div key={item.id} className="bg-white/5 p-3 rounded-2xl border border-white/10">
              <div className="flex justify-between items-start"><p className="font-bold text-xs uppercase flex-1">{item.nom}</p><button onClick={()=>setPanier(panier.filter(x=>x.id!==item.id))} className="text-[10px] opacity-50">✕</button></div>
              <div className="flex items-center gap-4 mt-2">
                <div className="flex items-center gap-2 bg-black/20 p-1 rounded-lg">
                    <button onClick={()=>setPanier(panier.map(x=>x.id===item.id?{...x, qte: Math.max(1, x.qte-1)}:x))}>-</button>
                    <span className="text-xs font-black">{item.qte}</span>
                    <button onClick={()=>setPanier(panier.map(x=>x.id===item.id?{...x, qte: x.qte+1}:x))}>+</button>
                </div>
                <div className="flex-1 flex items-center gap-2">
                    <span className="text-[9px] opacity-60">Remise:</span>
                    <input type="number" className="w-16 p-1 bg-black/30 rounded text-[10px] outline-none border border-white/10" value={item.remise_montant} onChange={e=>updateRemise(item.id, e.target.value)} />
                </div>
                <span className="font-black text-xs">{formatAr((item.prix_vente - item.remise_montant) * item.qte)} Ar</span>
              </div>
            </div>
          ))}
        </div>
        <div className="mt-4 pt-4 border-t border-white/20 space-y-3">
          ...
          <div className="flex gap-2">
              <div className="flex-1"><label className="text-[9px] font-bold opacity-50 block ml-1 uppercase">Remise Globale %</label><input type="number" className="w-full p-2 bg-black/20 rounded-lg outline-none font-bold" value={remiseGl} onChange={e=>setRemiseGl(Number(e.target.value))} /></div>
...
const AdminFournisseurs = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <h2 className="text-2xl font-black uppercase text-[#800020] border-b-2 border-red-100 pb-2">Fournisseurs</h2>
      <form onSubmit={saveNouveau} className="bg-white p-6 rounded-3xl shadow-sm grid grid-cols-1 md:grid-cols-3 gap-3">
        <input placeholder="Société" className="p-3 bg-gray-50 border rounded-xl" value={form.nom} onChange={e=>setForm({...form, nom: e.target.value})} required />
        <input placeholder="Tél Fixe" className="p-3 bg-gray-50 border rounded-xl" value={form.tel} onChange={e=>setForm({...form, tel: e.target.value})} required />
        <input placeholder="WhatsApp" className="p-3 bg-green-50 border border-green-100 rounded-xl" value={form.wa} onChange={e=>setForm({...form, wa: e.target.value})} />
        <button className="bg-[#800020] text-white p-3 rounded-xl font-black uppercase md:col-span-3">Ajouter Fournisseur</button>
      </form>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {list.map(f => (
          <div key={f.id} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
            <p className="font-black text-sm uppercase mb-2">{f.nom}</p>
            <div className="flex gap-2">
              <span className="flex-1 bg-gray-100 text-gray-600 p-2 rounded text-center text-[10px] font-bold">📞 {f.telephone}</span>
              {f.contact_whatsapp && <a href={`https://wa.me/${String(f.contact_whatsapp).replace(/[^0-9]/g, '')}`} target="_blank" className="flex-1 bg-green-500 text-white p-2 rounded text-center text-[10px] font-black">💬 WhatsApp</a>}
            </div>
          </div>
        ))}
      </div>
...
const ModuleJournalFactures = ({ params }) => {
  ...
  useEffect(() => { const load = async () => { const { data } = await supabase.from('historique_ventes').select('*').in('type_vente', ['FACTURE', 'FACTURE_A4']).order('date_vente', { ascending: false }); setFactures(data || []); }; load(); }, []);
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <h2 className="text-2xl font-black uppercase text-[#800020] border-b-2 border-[#800020] pb-2">Journal des Factures</h2>
      <div className="grid gap-3">{factures.map(v => (
        <div key={v.id} className="bg-white p-5 rounded-2xl shadow-sm border flex justify-between items-center">
          <div><p className="font-black text-[#800020]">{v.numero_facture}</p><p className="font-bold uppercase text-sm">{v.client_nom}</p><p className="text-[10px] text-gray-500 mt-1">🛒 {v.articles_liste}</p></div>
          <div className="text-right"><p className="text-xl font-black text-gray-800 mb-2">{formatAr(v.montant_total)} Ar</p><button onClick={() => lancerImpression('facture_a4', { ...v, panier: v.details_json?.articles || [], totalNet: v.montant_total, date: v.date_vente, printSize: 'A4' }, params)} className="bg-blue-100 text-blue-800 px-4 py-2 rounded-lg font-black text-xs">📄 PDF (A4)</button></div>
        </div>
      ))}</div>
    </div>
  );
...
const ModuleJournalDevis = ({ params }) => {
  ...
  const transform = async (d) => {
    if(!window.confirm(`Transformer en Facture ? Le stock sera déduit.`)) return;
    for (let art of d.details_json.articles) { await supabase.rpc('decrement_stock_by_name', { p_nom: art.nom, amount: safeNum(art.qte) }); }
    await supabase.from('historique_ventes').insert([{ numero_facture: `FA${Date.now().toString().slice(-6)}`, type_vente: 'FACTURE', client_nom: d.client_nom, articles_liste: d.articles_liste, montant_total: d.montant_total, details_json: d.details_json, methode_paiement: 'CASH' }]);
    await supabase.from('devis').update({ statut: 'Facturé ✅' }).eq('id', d.id);
    load(); alert("Transformé !");
  };
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="grid gap-3">{devis.map(d => (
        <div key={d.id} className="bg-white p-5 rounded-2xl shadow-sm border flex justify-between items-center">
          <div><p className="font-black text-[#800020]">{d.numero_devis}</p><span className={`text-[10px] font-bold px-2 py-0.5 rounded ${d.statut === 'En attente' ? 'bg-orange-100 text-orange-700' : 'bg-green-100 text-green-700'}`}>{d.statut || 'En attente'}</span><p className="font-bold text-sm uppercase mt-1">{d.client_nom}</p></div>
          <div className="text-right"><p className="text-xl font-black text-gray-800 mb-2">{formatAr(d.montant_total)} Ar</p>{d.statut !== 'Facturé ✅' && <button onClick={()=>transform(d)} className="bg-green-600 text-white px-4 py-2 rounded-lg font-black text-xs">🔄 Facturer</button>}</div>
        </div>
      ))}</div>
    </div>
  );
...
const AdminDashboard = () => {
  const [stats, setStats] = useState({ ca: 0, topP: [], topF: [], stock: 0 });
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const totalCA = v?.reduce((acc, x) => acc + safeNum(x.montant_total), 0) || 0;
      const stockVal = p?.reduce((acc, x) => acc + (safeNum(x.prix_achat)*safeNum(x.stock_actuel)), 0) || 0;

      let counts = {};
      let foursCounts = {};
      v?.forEach(sale => {
        if(sale.details_json?.articles) {
          sale.details_json.articles.forEach(art => {
            counts[art.nom] = (counts[art.nom] || 0) + safeNum(art.qte);
            const pInfo = p?.find(prod => prod.nom === art.nom);
            if(pInfo) { foursCounts[pInfo.fournisseur_nom] = (foursCounts[pInfo.fournisseur_nom] || 0) + safeNum(art.qte); }
          });
        }
      });
      const topP = Object.entries(counts).sort((a,b) => b[1]-a[1]).slice(0, 5);
      const topF = Object.entries(foursCounts).sort((a,b) => b[1]-a[1]).slice(0, 3);

      setStats({ ca: totalCA, topP, topF, stock: stockVal });
    };
    load();
  }, []);
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <h2 className="text-2xl font-black uppercase text-[#800020] border-b-2 border-red-100 pb-2">Tableau de Bord</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-3xl shadow-sm border-l-8 border-[#800020]"><p className="text-xs font-bold text-gray-400 uppercase">Chiffre d'Affaires Global</p><p className="text-3xl font-black">{formatAr(stats.ca)} Ar</p></div>
        <div className="bg-white p-6 rounded-3xl shadow-sm border-l-8 border-blue-500"><p className="text-xs font-bold text-gray-400 uppercase">Valeur du Stock</p><p className="text-3xl font-black text-blue-600">{formatAr(stats.stock)} Ar</p></div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
          <h3 className="font-black text-xs uppercase mb-4 text-[#800020]">🏆 Top 5 Produits</h3>
          {stats.topP.map(([nom, qte], i) => (
            <div key={i} className="flex justify-between py-2 border-b last:border-0"><span className="text-sm font-bold uppercase">{nom}</span><span className="font-black">{qte} pcs</span></div>
          ))}
        </div>
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
          <h3 className="font-black text-xs uppercase mb-4 text-blue-600">🚚 Top Fournisseurs</h3>
          {stats.topF.map(([nom, qte], i) => (
            <div key={i} className="flex justify-between py-2 border-b last:border-0"><span className="text-sm font-bold uppercase">{nom || 'Inconnu'}</span><span className="font-black text-blue-600">{qte} pts</span></div>
          ))}
        </div>
      </div>
    </div>
          ...
          <div key={c.id} className="bg-white p-6 rounded-[2rem] shadow-sm border-l-[12px] border-red-600 flex flex-col md:flex-row justify-between items-center gap-6">
            ...
            <div className="text-center md:text-right w-full md:w-auto border-t md:border-t-0 pt-4 md:pt-0">
                ...
                <div className="flex gap-2 justify-center md:justify-end">
                    ...
                    <button onClick={()=>{window.open(`https://wa.me/?text=Bonjour ${c.nom_client}, c'est Hakimi Plus. Petit rappel concernant votre dette de ${formatAr(c.montant_du)} Ar. Merci !`)}} className="bg-blue-500 text-white p-3 rounded-2xl shadow-lg font-black">💬 WhatsApp</button>
                </div>
            </div>
          </div>
        ))}
