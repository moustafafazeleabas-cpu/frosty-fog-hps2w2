  ...
  useEffect(() => {
    if (user) {
      ...
      const checkTasks = async () => {
        ...
        const now2 = new Date(); const h = now2.getHours(); const isLockTime = (h >= 18 || h < 7);
        if (isLockTime) {
           let lockWindowStart = new Date(now2); if (h < 7) lockWindowStart.setDate(lockWindowStart.getDate() - 1); lockWindowStart.setHours(18, 0, 0, 0);
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row font-sans text-gray-800">
      ...
      <nav className={`fixed inset-y-0 left-0 z-[70] w-72 bg-[#800020] text-white p-6 shadow-2xl flex flex-col justify-between overflow-y-auto transition-transform transform ${menuOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0`}>
        <div>
          ...
          <div className="flex flex-col gap-1">
            ...
            <NavBtn active={view==='messagerie'} onClick={()=>changeView('messagerie')} disabled={isLocked}><span className="flex items-center justify-between w-full"><span>✉️ Messagerie</span>{msgNonLus > 0 && <span className="bg-red-500 text-white px-2 py-0.5 rounded-full text-[10px] animate-pulse">{msgNonLus}</span>}</span></NavBtn>
      ...
      <main className="flex-1 p-4 md:p-6 lg:p-8 overflow-y-auto relative">
        {isLocked && view !== 'cloture' && (<div className="absolute inset-0 bg-white/80 backdrop-blur-sm z-50 flex flex-col items-center justify-center"><div className="text-6xl mb-4 animate-bounce">🔒</div><h2 className="text-2xl font-black text-[#800020] uppercase text-center">Accès Verrouillé</h2><p className="font-bold text-gray-500 mt-2 text-center px-4">Il est temps de clôturer la caisse.<br/>Veuillez effectuer la clôture pour continuer.</p></div>)}
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 h-full">
      <div className="bg-white p-4 rounded-3xl shadow-sm border-t-4 border-[#800020] flex flex-col h-[50vh] xl:h-[85vh]">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-2 gap-2 overflow-y-auto pr-1 custom-scrollbar">{produitsFiltres.map(p => (<button key={p.id} onClick={() => ajouter(p)} disabled={venteReussie} className="flex flex-col justify-between p-3 border border-gray-200 rounded-xl text-left bg-white hover:border-[#800020] hover:shadow-md transition group min-h-[85px]"><div className="flex justify-between items-start w-full gap-1 mb-1"><div><p className="font-bold text-gray-800 text-[11px] uppercase truncate group-hover:text-[#800020]">{p.nom}</p><p className="text-[8px] text-gray-400 font-bold uppercase">{p.categorie || 'Divers'}</p></div><span className={`px-1.5 py-0.5 rounded text-[9px] font-black shrink-0 ${safeNum(p.stock_actuel) <= 0 ? 'bg-red-600 text-white animate-pulse' : 'bg-gray-100 text-gray-500'}`}>STK: {p.stock_actuel}</span></div><p className="text-red-600 font-black text-sm">{formatAr(p.prix_vente)} Ar</p></button>))}</div>
      </div>
...
const ModuleCloture = ({ user, onClotureDone }) => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      <div className={`bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-gray-200 h-fit ${clotureOk ? 'opacity-50 pointer-events-none' : ''}`}><h3 className="text-lg font-black uppercase text-gray-800 mb-2">💸 Sorties Exceptionnelles</h3><p className="text-[10px] text-gray-500 mb-6 uppercase font-bold">Achats de matériels, urgences.</p><form onSubmit={declarerSortie} className="space-y-3 mb-6 bg-gray-50 p-4 rounded-2xl border border-gray-100"><input placeholder="Motif (ex: Cadenas)" className="w-full p-3 border rounded-xl text-sm outline-none" value={formSortie.motif} onChange={e=>setFormSortie({...formSortie, motif: e.target.value})} required disabled={uploading}/><input type="number" placeholder="Montant (Ar)" className="w-full p-3 border rounded-xl text-sm font-bold text-red-600 outline-none" value={formSortie.montant} onChange={e=>setFormSortie({...formSortie, montant: e.target.value})} required disabled={uploading} /><div className="text-xs"><label className="font-bold text-gray-500 block mb-1">Preuve / Facture (Opt) :</label><input type="file" accept="image/*" onChange={e=>setFormSortie({...formSortie, fichier: e.target.files[0]})} className="text-[10px]" disabled={uploading} /></div><button className="w-full bg-gray-800 text-white p-3 rounded-xl font-black text-xs uppercase hover:bg-black transition">{uploading ? 'En cours...' : 'Déclarer'}</button></form><div className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar">{sortiesListe.map(s => (<div key={s.id} className="p-3 bg-white border border-red-100 rounded-xl flex justify-between items-center shadow-sm"><div><p className="font-bold text-xs uppercase text-gray-800">{s.motif}</p>{s.photo_url && <a href={s.photo_url} target="_blank" rel="noreferrer" className="text-[9px] text-blue-600 underline font-bold">📄 Voir</a>}</div><span className="font-black text-red-600 text-sm">-{formatAr(s.montant)}</span></div>))}</div></div>
      <div className="bg-white p-6 md:p-8 rounded-3xl shadow-xl border-t-8 border-[#800020] text-center flex flex-col justify-center relative"><h2 className="text-2xl font-black uppercase italic tracking-tighter text-[#800020] mb-1">Clôture du Shift</h2><p className="text-[9px] text-gray-400 font-bold uppercase mb-6">Prise en compte des ventes depuis la dernière clôture.</p>
        {!clotureOk ? (
          <div className="space-y-4"><div className="grid grid-cols-3 gap-2 mb-4"><div className="bg-green-50 p-3 rounded-xl border border-green-200"><p className="text-[9px] font-bold text-green-700 uppercase">MVola</p><p className="text-sm md:text-base font-black text-green-800">{formatAr(caMvola)}</p></div><div className="bg-orange-50 p-3 rounded-xl border border-orange-200"><p className="text-[9px] font-bold text-orange-700 uppercase">Orange</p><p className="text-sm md:text-base font-black text-orange-800">{formatAr(caOM)}</p></div><div className="bg-pink-50 p-3 rounded-xl border border-pink-200"><p className="text-[9px] font-bold text-pink-700 uppercase">Chèques</p><p className="text-sm md:text-base font-black text-pink-800">{formatAr(caCheque)}</p></div></div><div className="bg-blue-50 p-6 rounded-2xl border border-blue-200 relative"><p className="text-xs font-black text-blue-800 uppercase tracking-widest mb-1">Espèces Attendues</p><p className="text-4xl font-black text-blue-900 tracking-tighter">{formatAr(caCash)} Ar</p>{totalSorties > 0 && <span className="absolute top-2 right-2 bg-red-500 text-white text-[9px] px-2 py-1 rounded font-black">- {formatAr(totalSorties)} sorties</span>}</div><div className="pt-4 border-t border-gray-100 text-left"><label className="block text-[11px] font-black text-red-600 uppercase mb-2 text-center">Billet physiquement présents ?</label><input type="number" className="w-full text-center text-2xl p-4 bg-red-50 border border-red-200 rounded-xl outline-none font-black text-red-700 placeholder-red-300" placeholder="0" value={montantDeclare} onChange={e => setMontantDeclare(e.target.value)} /></div>{montantDeclare !== '' && ecartEnCours !== 0 && (<div className="animate-pulse bg-orange-50 p-3 rounded-xl border border-orange-300 text-left"><p className="text-[10px] font-black text-orange-800 uppercase mb-1">⚠️ Écart détecté ({formatAr(ecartEnCours)} Ar) - Justification OBLIGATOIRE</p><input type="text" className="w-full p-2 text-xs border rounded outline-none" placeholder="Pourquoi y a-t-il une différence ?" value={justification} onChange={e=>setJustification(e.target.value)} required /></div>)}<button onClick={validerCloture} className="w-full bg-[#800020] text-white p-4 rounded-xl font-black uppercase shadow-md hover:bg-[#5a0016] transition mt-2">Valider Définitivement</button></div>
...
const ModuleJournalDevis = ({ params }) => {
  ...
  const reImprimer = (d) => { if (d.statut === 'Facturé ✅') { const dataPrint = { numero: d.numero_facture_liee, client_nom: d.client_nom, date: d.date_devis, totalNet: d.montant_total, totalRemisesEnAr: d.total_remise_ar, panier: d.details_json?.articles || [], fraisLivraison: safeNum(d.details_json?.frais_livraison) }; lancerImpression('facture_a4', dataPrint, params); } else { const dataPrint = { numero: d.numero_devis, client_nom: d.client_nom, date: d.date_devis, totalNet: d.montant_total, totalRemisesEnAr: d.total_remise_ar, panier: d.details_json?.articles || [], fraisLivraison: safeNum(d.details_json?.frais_livraison) }; lancerImpression('devis', dataPrint, params); } };
...
const AdminFournisseurs = () => {
  ...
  const save = async (e) => { e.preventDefault(); await supabase.from('fournisseurs').insert([{ nom: form.nom, telephone: form.tel, contact_whatsapp: form.wa }]); setForm({ nom: '', tel: '', wa: '' }); load(); };
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <form onSubmit={save} className="bg-white p-6 rounded-3xl shadow-sm grid grid-cols-1 md:grid-cols-3 gap-3 border-t-4 border-[#800020]"><input placeholder="Société" className="p-3 bg-gray-50 border rounded-xl outline-none" value={form.nom} onChange={e=>setForm({...form, nom: e.target.value})} required /><input placeholder="Tél Fixe" className="p-3 bg-gray-50 border rounded-xl outline-none" value={form.tel} onChange={e=>setForm({...form, tel: e.target.value})} required /><input placeholder="WhatsApp" className="p-3 bg-green-50 border border-green-100 rounded-xl outline-none" value={form.wa} onChange={e=>setForm({...form, wa: e.target.value})} /><button className="bg-[#800020] text-white p-3 rounded-xl font-black uppercase md:col-span-3">Ajouter</button></form>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">{list.map(f => (<div key={f.id} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100"><p className="font-black text-sm uppercase text-gray-800 mb-2">{f.nom}</p><div className="flex gap-2"><span className="flex-1 bg-gray-100 text-gray-600 p-2 rounded text-center text-[10px] font-bold">📞 {f.telephone}</span>{f.contact_whatsapp && <a href={`https://wa.me/${String(f.contact_whatsapp).replace(/[^0-9]/g, '')}`} target="_blank" rel="noreferrer" className="flex-1 bg-green-500 text-white p-2 rounded text-center text-[10px] font-black hover:bg-green-600">💬 WhatsApp</a>}</div></div>))}</div>
    </div>
  );
};
