...
export default function App() {
  ...
  useEffect(() => {
    if (user) {
      if(user.role === 'vendeur') setView('caisse'); // Le vendeur arrive direct en caisse
      ...
      <nav className={`${menuOpen ? 'block' : 'hidden'} md:block w-full md:w-72 bg-[#800020] text-white p-6 shadow-2xl flex flex-col justify-between md:sticky md:top-0 md:h-screen overflow-y-auto z-40 transition-all custom-scrollbar`}>
        <div>
          ...
          <div className="flex flex-col gap-1">
            ...
            <NavBtn active={view==='journal_factures'} onClick={()=>changeView('journal_factures')}>🧾 Journal des Factures</NavBtn>
            <NavBtn active={view==='journal_devis'} onClick={()=>changeView('journal_devis')}>📚 Journal des Devis</NavBtn>
            <NavBtn active={view==='historique'} onClick={()=>changeView('historique')}>📅 Historique Caisse</NavBtn>
            ...
            {user.role === 'superadmin' && (
              <div className="mt-4 pt-4 border-t border-white/10 flex flex-col gap-1">
                ...
                <NavBtn active={view==='parametres'} onClick={()=>changeView('parametres')}>⚙️ Paramètres ERP</NavBtn>
              </div>
            )}
          </div>
        </div>
...
// ==========================================
// 1. MODULE VENTE (AVEC MÉTHODE DE PAIEMENT)
// ==========================================
const ModuleVente = ({ mode, params }) => {
  ...
  const [methodePaiement, setMethodePaiement] = useState("CASH"); // Cash, Mvola, OM
  ...
  const valider = async () => {
    ...
    if (mode === 'devis') {
      ...
      const pMethode = mode === 'caisse' ? methodePaiement : 'CASH'; // Factures/Credits comptés comme CASH par défaut ou modifiables plus tard
    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-2xl shadow-sm border-l-4 border-blue-500">
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">CA Aujourd'hui</p>
          <p className="text-2xl font-black text-gray-800 mt-1">{formatAr(caJour)} Ar</p>
        </div>
        <div className="bg-white p-5 rounded-2xl shadow-sm border-l-4 border-[#800020]">
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">CA Ce Mois</p>
          <p className="text-2xl font-black text-[#800020] mt-1">{formatAr(caMois)} Ar</p>
        </div>
        <div className="bg-red-50 p-5 rounded-2xl shadow-sm border-l-4 border-red-500">
          <p className="text-[10px] font-bold text-red-500 uppercase tracking-widest">Charges (Mois)</p>
          <p className="text-2xl font-black text-red-700 mt-1">-{formatAr(depMois)} Ar</p>
        </div>
        <div className="bg-green-700 p-5 rounded-2xl shadow-md text-white">
          <p className="text-[10px] font-bold text-green-200 uppercase tracking-widest">Bénéfice Net (Mois)</p>
          <p className="text-2xl font-black mt-1">{formatAr(benBrutMois - depMois)} Ar</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex flex-col justify-between">
          ...
          <div className="space-y-3">
             <div className="flex justify-between items-center"><span className="text-xs font-bold text-gray-500">💵 Espèces (Cash)</span><span className="font-black text-sm">{formatAr(cashMois)} Ar</span></div>
             <div className="w-full bg-gray-100 rounded-full h-1.5"><div className="bg-blue-500 h-1.5 rounded-full" style={{width: caMois>0 ? `${(cashMois/caMois)*100}%` : '0%'}}></div></div>
             
             <div className="flex justify-between items-center mt-2"><span className="text-xs font-bold text-gray-500">🟢 MVola</span><span className="font-black text-sm">{formatAr(mvolaMois)} Ar</span></div>
             <div className="w-full bg-gray-100 rounded-full h-1.5"><div className="bg-green-500 h-1.5 rounded-full" style={{width: caMois>0 ? `${(mvolaMois/caMois)*100}%` : '0%'}}></div></div>

             <div className="flex justify-between items-center mt-2"><span className="text-xs font-bold text-gray-500">🟠 Orange Money</span><span className="font-black text-sm">{formatAr(omMois)} Ar</span></div>
             <div className="w-full bg-gray-100 rounded-full h-1.5"><div className="bg-orange-500 h-1.5 rounded-full" style={{width: caMois>0 ? `${(omMois/caMois)*100}%` : '0%'}}></div></div>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex flex-col justify-center items-center text-center">
          ...
          {produitsAlerte > 0 && <p className="mt-3 text-xs font-bold text-red-600 bg-red-50 px-3 py-1 rounded-full animate-pulse">⚠️ {produitsAlerte} articles en rupture imminente</p>}
        </div>
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex flex-col justify-center items-center text-center">
          ...
          <p className="mt-3 text-[10px] font-bold text-gray-400 uppercase">Dettes non payées à récupérer</p>
        </div>
      </div>
    </div>
  );
...
// ==========================================
// 4. JOURNAL DES FACTURES (NOUVEAU DÉDIÉ)
// ==========================================
const ModuleJournalFactures = ({ params }) => {
  ...
  const reImprimer = (v) => {
    const dataPrint = { numero: v.numero_facture, client_nom: v.client_nom, date: v.date_vente, totalNet: v.montant_total, totalRemisesEnAr: v.total_remise_ar, panier: v.details_json?.articles || [] };
    lancerImpression('facture_a4', dataPrint, params);
  };
          ...
          <div key={v.id} className="bg-white p-4 rounded-xl shadow-sm border flex flex-col md:flex-row justify-between items-center gap-3">
            <div className="flex-1 w-full cursor-pointer" onClick={() => setDetailModal(v)}>
              <div className="flex items-center gap-2 mb-1">
                ...
                {v.type_vente === 'CASH' && <BadgePaiement methode={v.methode_paiement} />}
...
const ModuleCloture = ({ user }) => {
  ...
  const loadData = async () => {
    ...
    const { data: v } = await supabase.from('historique_ventes').select('montant_total, methode_paiement').gte('date_vente', startIso).eq('type_vente', 'CASH');
    ...
    (v || []).forEach(x => {
       ...
       else tCash += safeNum(x.montant_total); // Default CASH
    });
