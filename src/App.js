...
// --- 🛡️ FONCTIONS GLOBALES ANTI-CRASH MATHÉMATIQUES ---
const safeNum = (val) => {
...
const lancerImpression = (type, data, params) => {
  ...
  const dateDoc = new Date(data.date || new Date()).toLocaleString();
  ...
  if (type === 'caisse') {
    win.document.write(`
      ...
      <body>
        ...
        <h2 style="margin:0;">${params.nom_entreprise}</h2>
        ...
        <p style="margin-top:10px;">${params.message_ticket}</p>
      </body></html>
    `);
  } else {
    const titre = type === 'devis' ? 'PROFORMA / DEVIS' : (type === 'admin_credit' ? 'FACTURE À CRÉDIT' : 'FACTURE');
    win.document.write(`
      <html><head><style>
      ...
      <body>
        <div class="header-flex">
          <div>
            ...
            <h3 style="margin:0; color:#800020;">${params.nom_entreprise}</h3>
          ...
          <div style="text-align:right;">
            ...
            <p style="margin:5px 0 0 0;">Date : ${new Date(data.date || new Date()).toLocaleDateString()}</p>
          </div>
        </div>
        <div class="client-box">
          ...
          ${type === 'admin_credit' && data.echeance ? `<br/><strong style="color:red;">Échéance : ${new Date(data.echeance).toLocaleDateString()}</strong>` : ''}
        </div>
        <table>
...
export default function App() {
  ...
  useEffect(() => {
    if (user) {
      ...
      checkMsgs(); const interval = setInterval(checkMsgs, 30000); return () => clearInterval(interval);
    }
  }, [user]);
      ...
      <nav className={`${menuOpen ? 'block' : 'hidden'} md:block w-full md:w-72 bg-[#800020] text-white p-6 shadow-2xl flex flex-col justify-between md:sticky md:top-0 md:h-screen overflow-y-auto z-40 transition-all custom-scrollbar`}>
        <div>
          ...
          <div className="flex flex-col gap-1">
            <NavBtn active={view==='messagerie'} onClick={()=>changeView('messagerie')}>✉️ Messagerie {msgNonLus > 0 && <span className="bg-red-500 text-white ml-2 px-2 py-0.5 rounded-full text-[10px] animate-pulse">{msgNonLus}</span>}</NavBtn>
            ...
            <NavBtn active={view==='journal_devis'} onClick={()=>changeView('journal_devis')}>📚 Journal des Devis</NavBtn>
...
// ==========================================
// 1. MODULE VENTE (AVEC NUMÉROTATION AUTO)
// ==========================================
const ModuleVente = ({ mode, params }) => {
  ...
  const [venteReussie, setVenteReussie] = useState(null); // Stocke les données pour l'impression
  ...
  useEffect(() => {
    ...
    load(); setPanier([]); setVenteReussie(null); setRemiseGlobale("");
  }, [mode]);
  ...
  const valider = async () => {
    ...
    // Génération du Numéro (FA260326-001 ou DV260326-001)
    let numero_genere = "";
    ...
    const detailsObj = {
      heure: today.toLocaleTimeString(),
    ...
    if (mode === 'devis') {
      ...
      await supabase.from('historique_ventes').insert([{
        numero_facture: numero_genere, type_vente: mode.replace('admin_', '').toUpperCase(), client_nom: selectedClient,
    ...
    // Preparer la donnée pour l'impression locale
    ...
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 h-full">
      ...
      <div className={`p-4 md:p-6 rounded-[2.5rem] shadow-xl flex flex-col justify-between relative overflow-hidden ${mode === 'devis' ? 'bg-white border-4 border-[#800020]' : 'bg-[#800020] text-white'} h-auto xl:h-[85vh]`}>
        ...
        <div className={`pt-4 border-t shrink-0 ${mode==='devis' ? 'border-gray-200' : 'border-white/20'}`}>
          ...
          {!venteReussie ? (<button onClick={valider} className={`w-full p-4 rounded-xl font-black uppercase text-sm shadow-lg transition ${mode === 'devis' ? 'bg-[#800020] text-white hover:bg-[#5a0016]' : 'bg-white text-[#800020] hover:bg-gray-200'}`}>{mode === 'devis' ? 'Générer Devis' : 'Valider'}</button>) : (
            <div className="flex gap-2">
              <button onClick={() => lancerImpression(mode, venteReussie, params)} className="flex-1 p-3 rounded-xl font-black uppercase bg-green-600 text-white shadow-lg hover:bg-green-700">🖨️ Imprimer {mode==='caisse'?'Ticket':(mode==='devis'?'Devis':'Facture')}</button>
              <button onClick={() => {setPanier([]); setVenteReussie(null); setRemiseGlobale(""); setSelectedClient(mode==='caisse' ? "Vente à un utilisateur" : "");}} className="flex-1 p-3 rounded-xl font-bold uppercase border border-white/50 text-white hover:bg-white/10">Nouveau</button>
            </div>
          )}
        </div>
      </div>
...
const ModuleJournalDevis = ({ params }) => {
  ...
  const transformerFacture = async (d) => {
    ...
    // Déduire stock et calculer bénefice
    let beneficeTotal = 0;
    if (d.details_json && d.details_json.articles) {
      for (let art of d.details_json.articles) {
         await supabase.rpc('decrement_stock_by_name', { p_nom: art.nom, amount: art.qte });
    ...
    // Insérer dans Ventes
    await supabase.from('historique_ventes').insert([{ numero_facture: numFacture, type_vente: 'FACTURE', client_nom: d.client_nom, articles_liste: d.articles_liste, montant_total: d.montant_total, benefice_total: beneficeTotal, remise_globale_pourcent: remiseGl, total_remise_ar: d.total_remise_ar, details_json: d.details_json }]);
    // Mettre à jour le statut du devis
    await supabase.from('devis').update({ statut: 'Facturé ✅' }).eq('id', d.id);
    load(); alert(`Transformé avec succès ! Numéro: ${numFacture}`);
  };
          ...
          <div key={d.id} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-200 flex flex-col md:flex-row justify-between items-center gap-4">
            <div className="flex-1 w-full">
              <div className="flex items-center gap-3 mb-1">
                ...
                <span className="text-[10px] text-gray-400 font-bold">{new Date(d.date_devis).toLocaleDateString()}</span>
              </div>
            ...
            <div className="flex gap-2 w-full md:w-auto shrink-0">
               <button onClick={()=>reImprimer(d)} className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-800 px-3 py-2 rounded-lg font-bold text-xs shadow-sm transition">🖨️ Imprimer</button>
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <form onSubmit={save} className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-gray-100 space-y-4">
        ...
        <div><label className="text-xs font-bold text-gray-500 uppercase">NIF / STAT (Ligne fiscale)</label><input className="w-full p-3 bg-gray-50 border rounded-xl outline-none" value={form.nif_stat||''} onChange={e=>setForm({...form, nif_stat: e.target.value})} /></div>
        <div><label className="text-xs font-bold text-gray-500 uppercase">Message fin de Ticket de Caisse</label><input className="w-full p-3 bg-gray-50 border rounded-xl outline-none italic" value={form.message_ticket||''} onChange={e=>setForm({...form, message_ticket: e.target.value})} /></div>
...
// ==========================================
// 4. HISTORIQUE VENTES (AJOUT IMPRESSION)
// ==========================================
const ModuleHistorique = ({ params }) => {
  ...
  const reImprimer = (v) => {
    const type = v.type_vente === 'CASH' ? 'caisse' : (v.type_vente === 'FACTURE' ? 'facture_a4' : 'admin_credit');
    const dataPrint = { numero: v.numero_facture, client_nom: v.client_nom, date: v.date_vente, totalNet: v.montant_total, totalRemisesEnAr: v.total_remise_ar, panier: v.details_json?.articles || [], printSize: '58mm' };
          ...
          <div key={v.id} className="bg-white p-4 rounded-xl shadow-sm border flex flex-col md:flex-row justify-between items-center gap-3">
            <div className="flex-1 w-full">
              <div className="flex items-center gap-2 mb-1">
                {v.numero_facture && <span className="font-black text-gray-800">{v.numero_facture}</span>}
                ...
                <span className="text-[10px] text-gray-400 font-bold">{new Date(v.date_vente).toLocaleDateString()}</span>
              </div>
            ...
            <button onClick={()=>reImprimer(v)} className="bg-gray-100 hover:bg-gray-200 text-gray-800 px-3 py-2 rounded-lg font-bold text-xs shadow-sm transition w-full md:w-auto shrink-0">🖨️ Re-imprimer</button>
          </div>
        ))}
      </div>
    </div>
...
// ==========================================
// 5. CLÔTURE CAISSE (INCHANGÉ MAIS COMPACTÉ)
// ==========================================
const ModuleCloture = ({ user }) => {
  const [caAttendu, setCaAttendu] = useState(0); const [totalSorties, setTotalSorties] = useState(0); const [montantDeclare, setMontantDeclare] = useState(''); const [clotureOk, setClotureOk] = useState(false); const [sortiesListe, setSortiesListe] = useState([]); const [formSortie, setFormSortie] = useState({ motif: '', montant: '', fichier: null }); const [uploading, setUploading] = useState(false);
  const loadData = async () => { const todayStart = new Date().toISOString().split('T')[0] + "T00:00:00.000Z"; const v = await supabase.from('historique_ventes').select('montant_total').gte('date_vente', todayStart).eq('type_vente', 'CASH'); const cash = v.data?.reduce((acc, x) => acc + safeNum(x.montant_total), 0) || 0; const s = await supabase.from('sorties_caisse').select('*').gte('date_sortie', todayStart); const sumSorties = s.data?.reduce((acc, x) => acc + safeNum(x.montant), 0) || 0; setSortiesListe(s.data || []); setTotalSorties(sumSorties); setCaAttendu(cash - sumSorties); }; useEffect(() => { loadData(); }, []);
  const validerCloture = async () => { if (montantDeclare === '') return alert("Saisissez le montant compté."); const ecart = safeNum(montantDeclare) - caAttendu; await supabase.from('cloture_caisse').insert([{ utilisateur: user.identifiant, montant_attendu: caAttendu, montant_declare: safeNum(montantDeclare), ecart: ecart }]); setClotureOk(true); };
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8"><div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-gray-200 h-fit"><h3 className="text-lg font-black uppercase text-gray-800 mb-2">💸 Sorties Exceptionnelles</h3><p className="text-xs text-gray-500 mb-6">Achats de matériels, urgences (déduits de la caisse).</p><form onSubmit={declarerSortie} className="space-y-3 mb-6 bg-gray-50 p-4 rounded-2xl border border-gray-100"><input placeholder="Motif (ex: Achat Cadenas)" className="w-full p-3 border rounded-xl text-sm outline-none" value={formSortie.motif} onChange={e=>setFormSortie({...formSortie, motif: e.target.value})} required disabled={uploading}/><input type="number" placeholder="Montant (Ar)" className="w-full p-3 border rounded-xl text-sm font-bold text-red-600 outline-none" value={formSortie.montant} onChange={e=>setFormSortie({...formSortie, montant: e.target.value})} required disabled={uploading} /><div className="text-xs"><label className="font-bold text-gray-500 block mb-1">Preuve / Facture (Optionnel) :</label><input type="file" accept="image/*" onChange={e=>setFormSortie({...formSortie, fichier: e.target.files[0]})} className="text-[10px]" disabled={uploading} /></div><button className="w-full bg-gray-800 text-white p-3 rounded-xl font-black text-xs uppercase hover:bg-black transition">{uploading ? 'Enregistrement...' : 'Déclarer la sortie'}</button></form><div className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar">{sortiesListe.map(s => (<div key={s.id} className="p-3 bg-white border border-red-100 rounded-xl flex justify-between items-center shadow-sm"><div><p className="font-bold text-xs uppercase text-gray-800">{s.motif}</p>{s.photo_url && <a href={s.photo_url} target="_blank" rel="noreferrer" className="text-[9px] text-blue-600 underline font-bold">📄 Voir justif.</a>}</div><span className="font-black text-red-600 text-sm">-{formatAr(s.montant)}</span></div>))}</div></div><div className="bg-white p-6 md:p-8 rounded-3xl shadow-xl border-t-8 border-[#800020] text-center flex flex-col justify-center"><h2 className="text-2xl md:text-3xl font-black uppercase italic tracking-tighter text-[#800020] mb-2">Clôture du Jour</h2>{!clotureOk ? (<div className="space-y-6 mt-4"><div className="bg-gray-50 p-6 rounded-2xl border border-gray-200 relative"><p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">À avoir en Caisse</p><p className="text-3xl md:text-4xl font-black text-gray-800 tracking-tighter">{formatAr(caAttendu)} Ar</p>{totalSorties > 0 && <span className="absolute top-2 right-2 bg-red-100 text-red-700 text-[9px] px-2 py-1 rounded font-black">- {formatAr(totalSorties)} de sorties</span>}</div><div className="pt-2"><label className="block text-xs font-bold text-red-600 uppercase mb-2">Argent réellement compté (Ar) :</label><input type="number" className="w-full text-center text-2xl p-4 bg-red-50 border border-red-200 rounded-xl outline-none font-black text-red-700" value={montantDeclare} onChange={e => setMontantDeclare(e.target.value)} /></div><button onClick={validerCloture} className="w-full bg-[#800020] text-white p-4 rounded-xl font-black uppercase shadow-md hover:bg-[#5a0016] transition mt-2">Soumettre la Clôture</button></div>) : (<div className="bg-green-50 border border-green-500 p-8 rounded-2xl mt-4"><h3 className="text-xl font-black text-green-700 uppercase mb-2">Caisse Clôturée ! ✅</h3><p className="font-bold text-gray-600 text-sm">Écart constaté : <span className={safeNum(montantDeclare) - caAttendu === 0 ? 'text-green-600' : 'text-red-600 font-black'}>{formatAr(safeNum(montantDeclare) - caAttendu)} Ar</span></p></div>)}</div></div>
  );
};

// ==========================================
// 6. MESSAGERIE INTERNE (INCHANGÉ)
// ==========================================
const ModuleMessagerie = ({ user }) => {
  const [messages, setMessages] = useState([]); const [form, setForm] = useState({ dest: user.role === 'superadmin' ? 'vendeur' : 'admin1996', obj: '', msg: '' });
  const load = async () => { const { data } = await supabase.from('messagerie').select('*').or(`destinataire.eq.${user.identifiant},expediteur.eq.${user.identifiant}`).order('date_envoi', { ascending: false }); setMessages(data || []); await supabase.from('messagerie').update({ est_lu: true }).eq('destinataire', user.identifiant).eq('est_lu', false); }; useEffect(() => { load(); }, []);
  const send = async (e) => { e.preventDefault(); await supabase.from('messagerie').insert([{ expediteur: user.identifiant, destinataire: form.dest, objet: form.obj, message: form.msg }]); setForm({ ...form, obj: '', msg: '' }); load(); alert("Message envoyé !"); };
  return (
    <div className="max-w-4xl mx-auto space-y-6"><div className="bg-white p-6 rounded-3xl shadow-sm border-t-4 border-[#800020]"><h2 className="text-sm font-black text-gray-400 uppercase tracking-widest mb-4">Nouveau Message</h2><form onSubmit={send} className="space-y-3"><div className="flex gap-3"><select className="p-3 bg-gray-50 border rounded-xl font-bold uppercase text-xs" value={form.dest} onChange={e=>setForm({...form, dest: e.target.value})}><option value="admin1996">Super Admin (Direction)</option><option value="vendeur">Vendeur (Caisse)</option></select><input placeholder="Objet du message..." className="flex-1 p-3 bg-gray-50 border rounded-xl outline-none" value={form.obj} onChange={e=>setForm({...form, obj: e.target.value})} required /></div><textarea placeholder="Écrivez votre message ici..." className="w-full p-4 bg-gray-50 border rounded-xl outline-none min-h-[100px] resize-none" value={form.msg} onChange={e=>setForm({...form, msg: e.target.value})} required /><button className="bg-[#800020] text-white px-6 py-3 rounded-xl font-black uppercase text-xs shadow-md">Envoyer 🚀</button></form></div><div className="space-y-3">{messages.map(m => (<div key={m.id} className={`p-5 rounded-2xl border ${m.expediteur === user.identifiant ? 'bg-gray-50 border-gray-200 ml-8' : 'bg-red-50 border-red-100 mr-8 shadow-sm'}`}><div className="flex justify-between items-start mb-2"><div><span className={`text-[10px] font-black uppercase px-2 py-1 rounded ${m.expediteur === user.identifiant ? 'bg-gray-200 text-gray-600' : 'bg-red-600 text-white'}`}>{m.expediteur === user.identifiant ? 'Moi' : m.expediteur}</span><span className="text-[10px] text-gray-400 font-bold ml-2">{new Date(m.date_envoi).toLocaleString()}</span></div></div><p className="font-black text-gray-800 text-sm mb-1">{m.objet}</p><p className="text-sm text-gray-600 leading-relaxed whitespace-pre-wrap">{m.message}</p></div>))}</div></div>
  );
};

// ==========================================
// 7. CLIENTS, FOURNISSEURS, CREDITS, STOCK... (INCHANGÉ, COMPACTÉ)
// ==========================================
