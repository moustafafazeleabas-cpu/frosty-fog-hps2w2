...
// ==========================================
// 1. MODULE VENTE (CORRIGÉ ET BLINDÉ ANTI-CRASH)
// ==========================================
const ModuleVente = ({ mode }) => {
  ...
  // CALCULS FINANCIERS SÉCURISÉS (Aucun crash possible)
  ...
  const ajouter = (p) => {
    ...
    else setPanier([...panier, { ...p, qte: 1, remise_montant: "" }]); // Initialise en string vide pour éviter le warning React
  };

  const updateRemiseArticle = (id, val) => {
    // val reste un string dans le state, la conversion se fait dans les calculs
  ...
  const imprimer = () => { 
    if (mode === 'caisse') {
      ...
      win.document.write(`<html><body style="font-family:monospace; width:${printSize}; padding:10px; font-size:12px; margin:0 auto; text-align:center;">
        ...
        <hr style="border-top:1px dashed #000;"/>
        <h3 style="text-align:right;">TOTAL: ${formatAr(totalNet)} Ar</h3>
        ${totalRemisesEnAr > 0 ? `<p style="text-align:right; font-size:10px;">(Dont remise : ${formatAr(totalRemisesEnAr)} Ar)</p>` : ''}
      ...
      win.document.write(`<html><body style="font-family:Arial; padding:40px;">
        ...
        <table style="width:100%; border-collapse:collapse; margin-top:20px;">
          ...
          <tbody>${panier.map(i => `<tr><td style="padding:12px; border-bottom:1px solid #eee;">${i.nom}</td><td style="padding:12px; border-bottom:1px solid #eee;">${i.qte}</td><td style="padding:12px; border-bottom:1px solid #eee;">${formatAr(Number(i.prix_vente) - (Number(i.remise_montant)||0))}</td><td style="padding:12px; border-bottom:1px solid #eee; text-align:right;">${formatAr((Number(i.prix_vente) - (Number(i.remise_montant)||0)) * i.qte)} Ar</td></tr>`).join('')}</tbody>
        </table>
        <div style="text-align:right; margin-top:30px; font-size:24px; font-weight:900; color:#800020;">TOTAL NET : ${formatAr(totalNet)} Ar</div>
        ${totalRemisesEnAr > 0 ? `<p style="text-align:right; font-size:12px; color:green;">(Remise globale appliquée : ${formatAr(totalRemisesEnAr)} Ar)</p>` : ''}
