...
export default function App() {
  // PERSISTANCE DE LA SESSION (On récupère l'utilisateur depuis le LocalStorage s'il existe)
  ...
  useEffect(() => {
    if (user) {
      ...
      const checkTasks = async () => {
        ...
        // Verrouillage Clôture (18h00 - 06h59)
...
const ModuleVente = ({ mode, params }) => {
  ...
  const [methodePaiement, setMethodePaiement] = useState("CASH"); // CASH, MVOLA, ORANGE MONEY, CHEQUE
      ...
      <div className="bg-white p-4 rounded-3xl shadow-sm border-t-4 border-[#800020] flex flex-col h-[50vh] xl:h-[85vh]">
        <div className="flex gap-2 mb-4">
          ...
          <select className="p-4 bg-gray-50 border border-gray-200 rounded-xl outline-none text-xs font-bold w-1/3" value={selectedCat} onChange={e => setSelectedCat(e.target.value)} disabled={venteReussie}>
            <option value="">Toutes Catégories</option>{CATEGORIES_PRODUITS.map(c => <option key={c} value={c}>{c}</option>)}
          </select>
        </div>
...
const AdminDashboard = () => {
  ...
  useEffect(() => { 
    const load = async () => { 
      // Chargement global (pour les stocks et créances qui ne dépendent pas des dates filtres)
      ...
      // Chargement filtré (Ventes et Dépenses)
  ...
  ventes.forEach(v => { 
    if (v.details_json && v.details_json.articles) {
       ...
       // Fallback ancienne méthode texte
       if(!v.articles_liste) return; v.articles_liste.split(', ').forEach(itemStr => { const match = itemStr.match(/^(\d+)x\s+(.+)$/); if(match) counts[match[2]] = (counts[match[2]] || 0) + parseInt(match[1]); });
    }
  });
...
// ==========================================
// 6. CLÔTURE CAISSE (RESTRICTIF + JUSTIFICATION)
// ==========================================
const ModuleCloture = ({ user, onClotureDone }) => {
  ...
  const loadData = async () => {
    ...
    // Vérifier si déjà clôturé pour ce shift
    const localCloture = localStorage.getItem('lastClotureDate');
    if (localCloture && new Date(localCloture) >= shiftStart) setClotureOk(true);
  };
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div className="bg-white p-6 md:p-8 rounded-3xl shadow-xl border-t-8 border-[#800020] text-center flex flex-col justify-center relative">
        ...
        {!clotureOk ? (
          ...
          <div className="bg-green-50 border border-green-500 p-8 rounded-2xl mt-4"><h3 className="text-xl font-black text-green-700 uppercase mb-2">Shift Clôturé ! ✅</h3><p className="font-bold text-gray-600 text-sm">Les opérations reprendront au prochain shift.</p></div>
        )}
      </div>
    </div>
  );
...
// ==========================================
// 7. JOURNAL DES CLÔTURES (NOUVEAU)
// ==========================================
      ...
      <div className="grid gap-3">
        {clotures.map(c => (
           <div key={c.id} onClick={()=>setModal(c)} className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex justify-between items-center cursor-pointer hover:border-[#800020] transition">
             <div>
                <p className="font-bold uppercase text-xs text-gray-500">{new Date(c.date_cloture).toLocaleString('fr-FR')}</p>
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50">
          <div className="bg-white p-6 rounded-3xl w-full max-w-sm shadow-2xl">
             ...
             <div className="space-y-4">
                <div><p className="text-[10px] uppercase font-bold text-gray-400">Date et Heure</p><p className="font-black">{new Date(modal.date_cloture).toLocaleString('fr-FR')}</p></div>
                ...
                {safeNum(modal.ecart) !== 0 && (
                   <div className="bg-orange-50 p-3 rounded-xl border border-orange-200"><p className="text-[10px] font-bold text-orange-800 uppercase mb-1">Justification du vendeur :</p><p className="text-xs italic text-orange-900">{modal.justification || 'Aucune justification (Ancien système)'}</p></div>
                )}
             </div>
          </div>
        </div>
...
// ==========================================
// 8. HISTORIQUE VENTES (INCLUANT CHÈQUE)
// ==========================================
