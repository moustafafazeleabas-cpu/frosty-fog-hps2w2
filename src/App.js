    ...
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 h-full">
      ...
      <div className={`p-4 md:p-6 rounded-[2.5rem] shadow-xl flex flex-col justify-between relative overflow-hidden ${mode === 'devis' ? 'bg-white border-4 border-[#800020]' : 'bg-[#800020] text-white'} h-auto xl:h-[85vh]`}>
        ...
        <div className={`pt-4 border-t shrink-0 ${mode==='devis' ? 'border-gray-200' : 'border-white/20'}`}>
          ...
          {!venteReussie ? (<button onClick={valider} className={`w-full p-4 rounded-xl font-black uppercase text-sm shadow-lg transition ${mode === 'devis' ? 'bg-[#800020] text-white hover:bg-[#5a0016]' : 'bg-white text-[#800020] hover:bg-gray-200'}`}>{mode === 'devis' ? 'Générer Devis' : 'Valider ('+methodePaiement+')'}</button>) : (<div className="flex flex-col md:flex-row gap-2"><button onClick={() => lancerImpression(mode, venteReussie, params)} className="flex-1 p-3 rounded-xl font-black uppercase bg-green-600 text-white shadow-lg hover:bg-green-700">🖨️ Imprimer</button><button onClick={async () => {
  // 🔄 1. On force le téléchargement du nouveau stock ultra-frais
  const { data: pData } = await supabase.from('produits').select('*').order('nom');
  if (pData) setProduits(pData);

  // 🧹 2. On vide la caisse pour le client suivant
  setPanier([]); 
  setVenteReussie(null); 
  setRemiseGlobale(""); 
  setSelectedClient(mode === 'caisse' ? "Vente à consommateur" : ""); 
  setMethodePaiement("CASH"); 
  setBanqueCheque(""); 
  setNumeroCheque(""); 
  setIsLivraison(false); 
  setFraisLivraison(1000);
}} className="flex-1 p-3 rounded-xl font-bold uppercase border border-white/50 text-white hover:bg-white/10">
  Nouveau
</button></div>)}
        </div>
      </div>
      ...
      <div className="bg-white p-4 md:p-8 rounded-3xl shadow-sm border-t-4 border-[#800020]">
        ...
        <form onSubmit={saveNouveau} className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="md:col-span-4">
            <label className="text-[10px] font-bold text-gray-400 uppercase">Description du produit (Visible sur le site Web)</label>
            <textarea className="w-full p-3 bg-gray-50 border rounded-xl outline-none text-xs min-h-[80px]" value={form.description} onChange={e=>setForm({...form, description: e.target.value})} disabled={isSubmitting} placeholder="Détails, ingrédients, dimensions, etc..."></textarea>
          </div>
<div className="md:col-span-2">
            <label className="text-[10px] font-bold text-orange-500 uppercase">Texte si rupture (ex: Dispo dans 24h)</label>
            <input type="text" className="w-full p-3 bg-orange-50 border border-orange-100 rounded-xl outline-none text-xs" value={form.texte_rupture} onChange={e=>setForm({...form, texte_rupture: e.target.value})} disabled={isSubmitting} placeholder="Laissez vide pour afficher 'Rupture de stock' par défaut"/>
          </div>
          <div className="md:col-span-2">
  ...
  const executerAnnulation = async (e) => { e.preventDefault(); if(isCancelling) return; setIsCancelling(true); const { data: admins } = await supabase.from('utilisateurs').select('*').eq('role', 'superadmin').eq('mot_de_passe', cancelPwd); if (!admins || admins.length === 0) { alert("⚠️ Code Superadmin incorrect !"); setIsCancelling(false); return; } // --- RESTITUTION SÉCURISÉE DU STOCK (Étape B) ---
    if (cancelModal.details_json && cancelModal.details_json.articles) { 
      for (let art of cancelModal.details_json.articles) { 
        ...
        await supabase.rpc('increment_stock_by_name', { p_nom: art.nom, amount: safeNum(art.qte) });

        // 2. Restituer la matière première si l'article annulé était un pack
        const { data: pDef } = await supabase.from('produits').select('est_pack, pack_produit_nom, pack_qte').eq('nom', art.nom).single();
        if (pDef && pDef.est_pack && pDef.pack_produit_nom) {
           const quantiteARestituer = safeNum(art.qte) * safeNum(pDef.pack_qte);
           await supabase.rpc('increment_stock_by_name', { p_nom: pDef.pack_produit_nom, amount: quantiteARestituer });
        }
      } 
    } 
