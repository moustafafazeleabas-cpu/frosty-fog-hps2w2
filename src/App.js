...
const lancerImpression = (type, data, params) => {
  ...
  if (type === 'caisse') {
    win.document.write(`
      ...
      <body>
        ...
        <table style="width:100%; text-align:left; font-size:11px;">
          ${data.panier.map(i => `<tr><td style="width:15%; font-weight:bold;">${safeNum(i.qte)}x</td><td style="width:50%;">${(i.nom||'').substring(0,15)}</td><td style="width:35%; text-align:right;">${formatAr((safeNum(i.prix_vente) - safeNum(i.remise_montant)) * safeNum(i.qte))}</td></tr>`).join('')}
        </table>
        <hr style="border-top:1px dashed #000;"/>
    ...
    win.document.write(`
      <html><head><style>
          @media print { @page { margin: 0; size: auto; } body { margin: 1cm; } } body { font-family: Arial, sans-serif; font-size: 13px; color: #333; } table { width: 100%; border-collapse: collapse; margin-top: 15px; } th, td { padding: 8px; border-bottom: 1px solid #eee; text-align: left; } th { background-color: #800020; color: white; } .header-flex { display: flex; justify-content: space-between; border-bottom: 2px solid #800020; padding-bottom: 15px; margin-bottom: 15px; } .client-box { background-color: #f9f9f9; border-left: 4px solid #800020; padding: 15px; width: 50%; margin-bottom: 20px; } .total-line { font-size: 20px; font-weight: bold; color: #800020; text-align: right; margin-top: 20px; }
        </style></head>
      <body>
        <div class="header-flex">
          <div><img src="${LOGO_URL}" style="height:50px; margin-bottom:5px;" onerror="this.style.display='none'"/><h3 style="margin:0; color:#800020;">${params.nom_entreprise || 'HAKIMI PLUS'}</h3><p style="margin:0; font-size:11px;">${params.adresse || ''}<br/>${params.nif_stat || ''}<br/>${params.contact || ''}</p></div>
          <div style="text-align:right;"><h2 style="margin:0; color:#800020; font-size: 22px;">${titre}</h2>${data.numero ? `<h3 style="margin:5px 0;">N° ${data.numero}</h3>` : ''}<p style="margin:5px 0 0 0;">Date : ${dateDoc}</p>${data.methode ? `<p style="margin:5px 0 0 0; font-weight:bold; font-size:11px;">Payé par : ${data.methode}${data.banque ? ` (${data.banque})` : ''}</p>` : ''}</div>
        </div>
        <div class="client-box"><strong>Client :</strong> ${data.client_nom}<br/><strong>NIF/STAT :</strong> ${data.client_nif || '-'}<br/>${data.client_tel ? `<strong>Contact :</strong> ${data.client_tel}<br/>` : ''}${type === 'admin_credit' && data.echeance ? `<br/><strong style="color:red;">Échéance : ${formatDate(data.echeance)}</strong>` : ''}</div>
        <table><thead><tr><th>Désignation</th><th>Qté</th><th>Prix U.</th><th style="text-align:right;">Total</th></tr></thead><tbody>${data.panier.map(i => `<tr><td>${i.nom}</td><td>${safeNum(i.qte)}</td><td>${formatAr(safeNum(i.prix_vente) - safeNum(i.remise_montant))}</td><td style="text-align:right;">${formatAr((safeNum(i.prix_vente) - safeNum(i.remise_montant)) * safeNum(i.qte))} Ar</td></tr>`).join('')}</tbody></table>
...
export default function App() {
  // PERSISTANCE DE CONNEXION (Garde la session même si on actualise F5)
  ...
  // Vérification système (Verrouillage DB, Alertes, Messagerie)
  useEffect(() => {
    if (user) {
      ...
      const checkTasks = async () => {
        ...
        // 3. VERROUILLAGE 100% BASE DE DONNÉES (18h00 - 06h59)
        ...
        if (isLockTime) {
           ...
           // On cherche dans la DB s'il y a eu une clôture depuis 18h aujourd'hui
           ...
           if (latestCloture && latestCloture.length > 0) {
               setIsLocked(false); // Clôture déjà faite pour ce soir
           } else {
               setIsLocked(true); setView('cloture'); // Bloqué !
           }
        } else {
           setIsLocked(false);
        }
  ...
  if (loading) return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center">
      ...
      <img src={LOGO_URL} alt="Chargement..." className="h-12 animate-pulse" onerror="this.style.display='none'" />
    </div>
  );
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row font-sans text-gray-800">
      ...
      <div className="md:hidden bg-[#800020] text-white p-4 flex justify-between items-center sticky top-0 z-50 shadow-md">
        ...
        <div className="flex items-center gap-4">
          <button onClick={() => setNotifOpen(!notifOpen)} className="text-xl relative">🔔 {totalAlertes > 0 && <span className="absolute -top-1 -right-2 bg-red-500 text-white rounded-full w-4 h-4 text-[10px] flex items-center justify-center font-black animate-pulse">{totalAlertes}</span>}</button>
      ...
      <nav className={`${menuOpen ? 'block' : 'hidden'} md:block w-full md:w-72 bg-[#800020] text-white p-6 shadow-2xl flex flex-col justify-between md:sticky md:top-0 md:h-screen overflow-y-auto z-40 transition-all custom-scrollbar`}>
        <div>
          <div className="mb-6 hidden md:flex flex-col items-center border-b border-white/10 pb-6 relative">
             <button onClick={() => setNotifOpen(!notifOpen)} className="absolute top-0 right-0 text-xl hover:scale-110 transition">🔔 {totalAlertes > 0 && <span className="absolute -top-1 -right-2 bg-red-500 text-white rounded-full w-5 h-5 text-[10px] flex items-center justify-center font-black animate-pulse shadow-lg">{totalAlertes}</span>}</button>
          ...
          <div className="flex flex-col gap-1">
            ...
            <NavBtn active={view==='cloture'} onClick={()=>setView('cloture')}>💰 Faire la Clôture</NavBtn> {/* Toujours cliquable */}
           ...
           <div className="p-2 overflow-y-auto custom-scrollbar">
             ...
             {alertesStockDLC.map((al, idx) => (
               <div key={idx} className={`p-3 rounded-xl mb-2 border ${al.type === 'stock' ? 'bg-red-50 border-red-100 text-red-800' : (al.type === 'dlc_depasse' ? 'bg-gray-800 text-white' : 'bg-orange-50 border-orange-100 text-orange-800')}`}>
                 <p className="font-bold text-xs uppercase">{al.type === 'stock' ? '⚠️ RUPTURE' : (al.type === 'dlc_depasse' ? '☠️ PÉRIMÉ' : '⏳ EXPIRATION PROCHE')}</p><p className="text-[11px] mt-1">{al.text}</p>
               </div>
             ))}
             {totalAlertes === 0 && <p className="text-center text-gray-400 text-xs py-4 italic">Aucune notification.</p>}
           </div>
        </div>
      )}
...
// ==========================================
// 1. MODULE VENTE (AVEC PAIEMENT PAR CHÈQUE)
// ==========================================
const ModuleVente = ({ mode, params }) => {
  ...
  const valider = async () => {
    ...
    const detailsObj = {
      ...
      articles: panier.map(i => ({ nom: i.nom, qte: safeNum(i.qte), prix_unitaire: safeNum(i.prix_vente), remise_unitaire_ar: safeNum(i.remise_montant), total_ligne: (safeNum(i.prix_vente) - safeNum(i.remise_montant)) * safeNum(i.qte) }))
    };
    ...
    if (mode === 'devis') {
      ...
      await supabase.from('historique_ventes').insert([{
        numero_facture: numero_genere, type_vente: dbTypeVente, client_nom: selectedClient, articles_liste: strArticles, montant_total: totalNet, benefice_total: beneficeNet,
        remise_globale_pourcent: safeNum(remiseGlobale), total_remise_ar: totalRemisesEnAr, details_json: detailsObj, methode_paiement: pMethode
      }]);
      ...
      <div className="bg-white p-4 rounded-3xl shadow-sm border-t-4 border-[#800020] flex flex-col h-[50vh] xl:h-[85vh]">
        <div className="flex gap-2 mb-4">
          ...
          <select className="p-4 bg-gray-50 border border-gray-200 rounded-xl outline-none text-xs font-bold w-1/3" value={selectedCat} onChange={e => setSelectedCat(e.target.value)} disabled={venteReussie}>
            <option value="">Catégories</option>{CATEGORIES_PRODUITS.map(c => <option key={c} value={c}>{c}</option>)}
          </select>
        </div>
        ...
        <div className={`pt-4 border-t shrink-0 ${mode==='devis' ? 'border-gray-200' : 'border-white/20'}`}>
               ...
               {methodePaiement === 'CHEQUE' && (
                 <div className="flex gap-2 mt-1">
                    <select className="flex-1 p-2 rounded text-black text-xs font-bold outline-none" value={banqueCheque} onChange={e=>setBanqueCheque(e.target.value)}>
                      <option value="">Sélectionner Banque...</option><option value="BOA">BOA</option><option value="BMOI">BMOI</option><option value="BRED">BRED</option><option value="BNI">BNI</option>
                    </select>
                    <input placeholder="N° Chèque (Optionnel)" className="flex-1 p-2 rounded text-black text-xs outline-none" value={numeroCheque} onChange={e=>setNumeroCheque(e.target.value)} />
                 </div>
               )}
             </div>
          )}
          {!venteReussie && panier.length > 0 && (<div className="flex justify-between items-center mb-3"><span className="text-xs font-bold uppercase opacity-70">Remise Globale (%) :</span><input type="number" min="0" className="w-16 p-1 text-center text-sm font-black text-black rounded outline-none" value={remiseGlobale !== undefined ? remiseGlobale : ''} onChange={e => setRemiseGlobale(e.target.value)} placeholder="0" /></div>)}
          ...
          {!venteReussie ? (<button onClick={valider} className={`w-full p-4 rounded-xl font-black uppercase text-sm shadow-lg transition ${mode === 'devis' ? 'bg-[#800020] text-white hover:bg-[#5a0016]' : 'bg-white text-[#800020] hover:bg-gray-200'}`}>{mode === 'devis' ? 'Générer Devis' : 'Valider ('+methodePaiement+')'}</button>) : (
            <div className="flex gap-2">
              ...
              <button onClick={() => {setPanier([]); setVenteReussie(null); setRemiseGlobale(""); setSelectedClient(mode==='caisse' ? "Vente à un utilisateur" : ""); setMethodePaiement("CASH"); setBanqueCheque(""); setNumeroCheque("");}} className="flex-1 p-3 rounded-xl font-bold uppercase border border-white/50 text-white hover:bg-white/10">Nouveau</button>
            </div>
          )}
        </div>
      </div>
...
// ==========================================
// 2. DASHBOARD AVEC FILTRE DE DATE + TOPS
// ==========================================
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white p-5 rounded-2xl shadow-sm border-l-4 border-[#800020]"><p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Chiffre d'Affaires Période</p><p className="text-2xl font-black text-[#800020] mt-1">{formatAr(caPeriode)} Ar</p></div>
        <div className="bg-gray-50 p-5 rounded-2xl shadow-sm border-l-4 border-gray-300"><p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Marge Brute (Bénéfice Ventes)</p><p className="text-2xl font-black text-gray-700 mt-1">{formatAr(benBrutPeriode)} Ar</p></div>
        <div className="bg-red-50 p-5 rounded-2xl shadow-sm border-l-4 border-red-500"><p className="text-[10px] font-bold text-red-500 uppercase tracking-widest">Charges (Dépenses)</p><p className="text-2xl font-black text-red-700 mt-1">-{formatAr(depPeriode)} Ar</p></div>
      </div>
      ...
      <div className="bg-green-700 p-6 rounded-2xl shadow-md text-white flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div><p className="text-xs font-bold text-green-200 uppercase tracking-widest mb-1">Bénéfice Net Réel de la période</p><p className="text-[10px] opacity-70">Marge Brute - Charges</p></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex flex-col">
          ...
          <div className="space-y-3 flex-1 justify-center flex flex-col">
             <div className="flex justify-between items-center"><span className="text-xs font-bold text-gray-500">💵 Cash</span><span className="font-black text-sm">{formatAr(cashP)} Ar</span></div><div className="w-full bg-gray-100 rounded-full h-1.5"><div className="bg-blue-500 h-1.5 rounded-full" style={{width: caPeriode>0 ? `${(cashP/caPeriode)*100}%` : '0%'}}></div></div>
             <div className="flex justify-between items-center mt-1"><span className="text-xs font-bold text-gray-500">🟢 MVola</span><span className="font-black text-sm">{formatAr(mvolaP)} Ar</span></div><div className="w-full bg-gray-100 rounded-full h-1.5"><div className="bg-green-500 h-1.5 rounded-full" style={{width: caPeriode>0 ? `${(mvolaP/caPeriode)*100}%` : '0%'}}></div></div>
             <div className="flex justify-between items-center mt-1"><span className="text-xs font-bold text-gray-500">🟠 Orange</span><span className="font-black text-sm">{formatAr(omP)} Ar</span></div><div className="w-full bg-gray-100 rounded-full h-1.5"><div className="bg-orange-500 h-1.5 rounded-full" style={{width: caPeriode>0 ? `${(omP/caPeriode)*100}%` : '0%'}}></div></div>
             <div className="flex justify-between items-center mt-1"><span className="text-xs font-bold text-gray-500">✍️ Chèques</span><span className="font-black text-sm">{formatAr(chequeP)} Ar</span></div><div className="w-full bg-gray-100 rounded-full h-1.5"><div className="bg-pink-500 h-1.5 rounded-full" style={{width: caPeriode>0 ? `${(chequeP/caPeriode)*100}%` : '0%'}}></div></div>
          </div>
        </div>
           ...
           <div className="bg-white p-5 rounded-3xl shadow-sm border border-gray-100 flex-1">
             ...
             <div className="space-y-2">
                {topFournisseurs.map((f, idx) => (
                   <div key={idx} className="flex justify-between text-xs"><span className="font-bold text-gray-600">{f[0]}</span><span className="font-black text-[#800020]">{f[1]} pts</span></div>
                ))}
...
// ==========================================
// 3. ADMIN STOCK (RÉPARÉ ET PROTÉGÉ)
// ==========================================
const AdminStock = () => { 
...
// ==========================================
// 4. JOURNAL DES DEVIS (RÉPARÉ ET PROTÉGÉ)
// ==========================================
...
// ==========================================
// 5. CLÔTURE CAISSE (SÉCURITÉ DB UNIQUE)
// ==========================================
const ModuleCloture = ({ user, onClotureDone }) => {
  ...
  const loadData = async () => {
    ...
    if (h >= 7 && h < 18) { setTropTot(true); return; } // Bloqué avant 18h

    let shiftStart = new Date(now); if (h < 7) { shiftStart.setDate(shiftStart.getDate() - 1); } shiftStart.setHours(7, 0, 0, 0);

    // On cherche la DERNIÈRE clôture de la DB pour ne compter que les ventes faites APRÈS
    ...
    // Si on a déjà clôturé dans CE shift (entre 18h et maintenant), on affiche Validé
    if (lastClotureData && new Date(lastClotureData.date_cloture) >= shiftStart && h >= 18) {
       setClotureOk(true);
    }

    // Le CA à calculer commence soit à 07h00, soit après la dernière clôture (si on a fait des ventes après 18h hier soir)
  ...
  const validerCloture = async () => {
    if (montantDeclare === '') return alert("Saisissez l'argent physique compté.");
    ...
    localStorage.setItem('lastClotureDate', new Date().toISOString()); // Gardé juste par confort immédiat
    setClotureOk(true);
  ...
  const declarerSortie = async (e) => {
    ...
    if (formSortie.fichier) { const fileExt = formSortie.fichier.name.split('.').pop(); const fileName = `justif_${Date.now()}.${fileExt}`; const { error: errUp } = await supabase.storage.from('justificatifs').upload(fileName, formSortie.fichier); if (!errUp) { const { data } = supabase.storage.from('justificatifs').getPublicUrl(fileName); publicUrl = data.publicUrl; } }
      ...
      <div className="bg-white p-6 md:p-8 rounded-3xl shadow-xl border-t-8 border-[#800020] text-center flex flex-col justify-center relative">
        ...
        {!clotureOk ? (
          <div className="space-y-4">
            ...
            <div className="bg-blue-50 p-6 rounded-2xl border border-blue-200 relative"><p className="text-xs font-black text-blue-800 uppercase tracking-widest mb-1">Espèces (Cash) Attendues</p><p className="text-4xl font-black text-blue-900 tracking-tighter">{formatAr(caCash)} Ar</p>{totalSorties > 0 && <span className="absolute top-2 right-2 bg-red-500 text-white text-[9px] px-2 py-1 rounded font-black">- {formatAr(totalSorties)} sorties</span>}</div>
          ...
          <div className="bg-green-50 border border-green-500 p-8 rounded-2xl mt-4"><h3 className="text-xl font-black text-green-700 uppercase mb-2">Shift Clôturé ! ✅</h3><p className="font-bold text-gray-600 text-sm">Les opérations reprennent au prochain shift.</p></div>
        )}
      </div>
    </div>
  );
